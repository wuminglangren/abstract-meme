...
class training_fonts_connection:
    ...
    def get_all(self, sorted_column = None, ascend_or_descend = None):

        select_query = '''SELECT * FROM FONTS'''


        column_list = ["utf_8", "abs_path", "font_name", "font_type"]

        if not(sorted_column is None):

            if sorted_column.lower().strip() in column_list:

                select_query += " ORDER BY " + sorted_column

                if ((ascend_or_descend == 1) or 
                    (ascend_or_descend.lower() == "ascend") or
                    (ascend_or_descend.lower() == "asc") or
                    (ascend_or_descend == None)):
                    
                    select_query += " ASC"

                elif((ascend_or_descend == -1) or
                     (ascend_or_descend.lower() == "descend") or
                     (ascend_or_descend.lower() == "desc")):

                     select_query += " DESC"
                else:

                    print("You input a wired value in \"ascend_or_descedn\", and we defaultly see it as ascend")
                    select_query += " ASC"
            ...
            pass
        elif ((sorted_column is None) and not(ascend_or_descend is None)):

            print("Please select specific column for sorting.")
    ...
    def delete_with_exact_value(self, utf_8 : int = None, abs_path : str = None, font_name : str = None, font_type : str = None) -> None:
        ...
        delete_query = "DELETE FROM FONTS"
        need_and = False
        filter_data = ()
        if (not(utf_8 is None) or
            not(abs_path is None) or
        ...
        delete_query += ";"
        self.cursor.execute(delete_query, filter_data)
        result = self.cursor.fetchall()
